...
Ekf15Dof<T>::Ekf15Dof(T sample_time_s, MatrixInv<T> initial_state, MatrixInv<T> process_noise_q, MatrixInv<T> meas_noise_r, MatrixInv<T> initial_covariance_p,
					  ...
					  meas_noise_r_orig_(meas_noise_r){	
...
template <typename T>
void Ekf15Dof<T>::PropagateState(const MatrixInv<T> &state_sensor_val, MatrixInv<T> &meas_sensor_val_corrected, const long long int meas_dt_us){
	// This function propogates the states and also corrects for gps meas delay
	// wrap the yaw angle	
	this->current_state_(2) = WrapToPi( this->current_state_(2) );
	...
	//Calculate dVn, dVe, dVd
	T dVn = ((s_phi*s_psi + c_phi*c_psi*s_theta)*(state_sensor_val(5) - this->current_state_(14)) - (c_phi*s_psi - c_psi*s_phi*s_theta)*(state_sensor_val(4) - this->current_state_(13)) + c_psi*c_theta*(state_sensor_val(3) - this->current_state_(12)));
	T dVe = ((c_phi*c_psi + s_phi*s_psi*s_theta)*(state_sensor_val(4) - this->current_state_(13)) - (c_psi*s_phi - c_phi*s_psi*s_theta)*(state_sensor_val(5) - this->current_state_(14)) + c_theta*s_psi*(state_sensor_val(3) - this->current_state_(12)));
	T dVd = (g(2) - s_theta*(state_sensor_val(3) - this->current_state_(12)) + c_phi*c_theta*(state_sensor_val(5) - this->current_state_(14)) + c_theta*s_phi*(state_sensor_val(4) - this->current_state_(13)));

	meas_sensor_val_corrected(3) += meas_dt_us*dVn;
	meas_sensor_val_corrected(4) += meas_dt_us*dVe;
	meas_sensor_val_corrected(5) += meas_dt_us*dVd;

	meas_sensor_val_corrected(0) += meas_dt_us*meas_sensor_val_corrected(3);
	meas_sensor_val_corrected(1) += meas_dt_us*meas_sensor_val_corrected(4);
	meas_sensor_val_corrected(2) += meas_dt_us*meas_sensor_val_corrected(5);
	...
	this->time_propagated_state_(9) = this->current_state_(9) + dt*dVn;

	this->time_propagated_state_(10) = this->current_state_(10) + dt*dVe;

	this->time_propagated_state_(11) = this->current_state_(11) + dt*dVd;
...
template <typename T>
void Ekf15Dof<T>::GetMeas(const MatrixInv<T> &meas_sensor_val, const bool meas_indices []){
	...
	//Assume we are in bay area and use a declination 
	 T magnetic_declination = 12.866667*DEG2RAD;
	...
	// printf("Heading [deg]: %g\n", this->computed_meas_(0)/DEG2RAD);
	...
	// modify the mag covariance base on measured roll and roll rate
	if( (this->current_state_(0)) > 5.0*DEG2RAD){
		this->meas_noise_r_(0, 0) = 10*meas_noise_r_orig_(0, 0);
	}else{
		this->meas_noise_r_(0, 0) = meas_noise_r_orig_(0, 0);
	}

	//Modify GPS meas covariance if gps validity is false
	if(!meas_indices[1]){
		this->meas_noise_r_(1, 1) = 100*meas_noise_r_orig_(1, 1);
		this->meas_noise_r_(2, 2) = 100*meas_noise_r_orig_(2, 2);
		this->meas_noise_r_(3, 3) = 100*meas_noise_r_orig_(3, 3);
		this->meas_noise_r_(4, 4) = 100*meas_noise_r_orig_(4, 4);
		this->meas_noise_r_(5, 5) = 100*meas_noise_r_orig_(5, 5);
		this->meas_noise_r_(6, 6) = 100*meas_noise_r_orig_(6, 6);
	}else{
		this->meas_noise_r_(1, 1) = meas_noise_r_orig_(1, 1);
		this->meas_noise_r_(2, 2) = meas_noise_r_orig_(2, 2);
		this->meas_noise_r_(3, 3) = meas_noise_r_orig_(3, 3);
		this->meas_noise_r_(4, 4) = meas_noise_r_orig_(4, 4);
		this->meas_noise_r_(5, 5) = meas_noise_r_orig_(5, 5);
		this->meas_noise_r_(6, 6) = meas_noise_r_orig_(6, 6);
	}
}
