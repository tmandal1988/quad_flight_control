...
template <typename T>
Ekf21Dof<T>::Ekf21Dof(T sample_time_s, MatrixInv<T> initial_state, MatrixInv<T> process_noise_q, MatrixInv<T> meas_noise_r):
...
template <typename T>
void Ekf21Dof<T>::ComputeTrignometricValues(){
...
template <typename T>
void Ekf21Dof<T>::PropagateState(MatrixInv<T> previous_state, MatrixInv<T> state_sensor_val){
	ComputeTrignometricValues();
	// propagate phi
	this->time_propagated_state_(0) = previous_state(0) + this->sample_time_s_*((state_sensor_val(0) - previous_state(3)) + 
																		s_phi*t_theta*(state_sensor_val(1) - previous_state(4)) +
																		c_phi*t_theta*(state_sensor_val(2) - previous_state(5))) ;

	// propagate theta
	this->time_propagated_state_(1) = previous_state(1) + this->sample_time_s_*( c_phi*(state_sensor_val(1) - previous_state(4)) - 
																		 s_phi*(state_sensor_val(2) - previous_state(5)) );

	// propagate psi
	this->time_propagated_state_(2) = previous_state(2) + this->sample_time_s_*( s_phi*sc_theta*(state_sensor_val(1) - previous_state(4)) - 
																		 c_phi*sc_theta*(state_sensor_val(2) - previous_state(5)) );
	...
	this->time_propagated_state_(9) = previous_state(9) + this->sample_time_s_*( c_theta*c_psi*(state_sensor_val(3) - previous_state(12)) +
																		 c_theta*s_psi*(state_sensor_val(4) - previous_state(13)) -
																		 s_theta*(state_sensor_val(5) - previous_state(14)) );
	// propagate NED Y velocity
	this->time_propagated_state_(10) = previous_state(10) + this->sample_time_s_*( (s_phi*s_theta*c_psi - c_phi*s_psi)*(state_sensor_val(3) - previous_state(12)) +
																		   (s_phi*s_theta*s_psi + c_phi*c_psi)*(state_sensor_val(4) - previous_state(13)) +
																		    s_phi*c_theta*(state_sensor_val(5) - previous_state(14)) );
	// propagate NED Z velocity
	this->time_propagated_state_(11) = previous_state(11) + this->sample_time_s_*( (c_phi*s_theta*c_psi + s_phi*s_psi)*(state_sensor_val(3) - previous_state(12)) +
																		   (c_phi*s_theta*s_psi - s_phi*c_psi)*(state_sensor_val(4) - previous_state(13)) +
																		    c_phi*c_theta*(state_sensor_val(5) - previous_state(14)) );
	...
	this->time_propagated_state_(15) = state_sensor_val(0) - previous_state(3);
	this->time_propagated_state_(16) = state_sensor_val(1) - previous_state(4);
	this->time_propagated_state_(17) = state_sensor_val(2) - previous_state(5);

	//filtered accel
	this->time_propagated_state_(18) = state_sensor_val(3) - previous_state(12);
	this->time_propagated_state_(19) = state_sensor_val(4) - previous_state(13);
	this->time_propagated_state_(20) = state_sensor_val(5) - previous_state(14);
}

template <typename T>
void Ekf21Dof<T>::ComputeStateJacobian(MatrixInv<T> previous_state, MatrixInv<T> state_sensor_val){

	// 1st row
	this->state_jacobian_(0, 0) = 1 + this->sample_time_s_*( c_phi*t_theta*(state_sensor_val(1) - previous_state(4)) -
															 s_phi*t_theta*(state_sensor_val(2) - previous_state(5)) );
	this->state_jacobian_(0, 1) = this->sample_time_s_*(s_phi*sc_theta*sc_theta*(state_sensor_val(1) - previous_state(4)) +
														c_phi*sc_theta*sc_theta*(state_sensor_val(2) - previous_state(5)) );
	...
	//2nd row
	this->state_jacobian_(1, 0) = this->sample_time_s_*(-s_phi*(state_sensor_val(1) - previous_state(4)) -
														 c_phi*(state_sensor_val(2) - previous_state(5)) );
	...
	//3rd row
	this->state_jacobian_(2, 0) = this->sample_time_s_*( c_phi*sc_theta*(state_sensor_val(1) - previous_state(4)) - 
														 s_phi*sc_theta*(state_sensor_val(2) - previous_state(5)) );
	this->state_jacobian_(2, 1) = this->sample_time_s_*( s_phi*sc_theta*t_theta*(state_sensor_val(1) - previous_state(4)) +
														 c_phi*sc_theta*t_theta*(state_sensor_val(2) - previous_state(5)) );
	...
	//10th row
	this->state_jacobian_(9, 1) = this->sample_time_s_*( -s_theta*c_psi*(state_sensor_val(3) - previous_state(12)) -
														 s_theta*s_psi*(state_sensor_val(4) - previous_state(13)) -
														 c_theta*(state_sensor_val(5) - previous_state(14)) );
	this->state_jacobian_(9, 2) = this->sample_time_s_*( -c_theta*s_psi*(state_sensor_val(3) - previous_state(12)) + 
														 c_theta*c_psi*(state_sensor_val(5) - previous_state(14)) );
	...
	//11th row
	this->state_jacobian_(10, 0) = this->sample_time_s_*( (c_phi*s_theta*c_psi + s_phi*s_psi)*(state_sensor_val(3) - previous_state(12)) + 
									 (c_phi*s_theta*s_psi - s_phi*c_psi)*(state_sensor_val(4) - previous_state(13)) + 
									  c_phi*c_theta*(state_sensor_val(5) - previous_state(14)) );
	this->state_jacobian_(10, 1) = this->sample_time_s_*( s_phi*c_theta*c_psi*(state_sensor_val(3) - previous_state(12)) - 
														  s_phi*c_theta*s_psi*(state_sensor_val(4) - previous_state(13)) -
														  s_phi*s_theta*(state_sensor_val(5) - previous_state(14)) );
	this->state_jacobian_(10, 2) = this->sample_time_s_*( (-s_phi*s_theta*s_psi - c_phi*c_psi)*(state_sensor_val(3) - previous_state(12)) +
														  (s_phi*s_theta*c_psi - c_phi*s_psi)*(state_sensor_val(4) - previous_state(13)) );
	...
	//12th row
	this->state_jacobian_(11, 0) = this->sample_time_s_*( (-s_phi*s_theta*c_psi + c_phi*s_psi)*(state_sensor_val(3) - previous_state(12)) +
														  (-s_phi*s_theta*s_psi - c_phi*c_psi)*(state_sensor_val(4) - previous_state(13)) - 
														  s_phi*c_theta*(state_sensor_val(5) - previous_state(14)) );
	this->state_jacobian_(11, 1) = this->sample_time_s_*( c_phi*c_theta*c_psi*(state_sensor_val(3) - previous_state(12)) +
														  c_phi*c_theta*s_psi*(state_sensor_val(4) - previous_state(13)) );
	this->state_jacobian_(11, 2) = this->sample_time_s_*( (-c_phi*s_theta*s_psi + s_phi*c_psi)*(state_sensor_val(3) - previous_state(12)) +
														  (c_phi*s_theta*c_psi + s_phi*s_psi)*(state_sensor_val(4) - previous_state(13)) );
...
template <typename T>
void Ekf21Dof<T>::ComputeStateNoiseJacobian(MatrixInv<T> previous_state){
	//1st row
	...
	//16th row
	this->state_noise_jacobian_(15, 15) = 1;
	//17th row
	this->state_noise_jacobian_(16, 16) = 1;
	//18th row
	this->state_noise_jacobian_(17, 17) = 1;

	//19th row
	this->state_noise_jacobian_(18, 18) = 1;
	//20th row
	this->state_noise_jacobian_(19, 19) = 1;
	//21st row
	this->state_noise_jacobian_(20, 20) = 1;


}

template <typename T>
void Ekf21Dof<T>::GetMeas(MatrixInv<T> meas_sensor_val){
...
template <typename T>
void Ekf21Dof<T>::ComputeMeasJacobian(MatrixInv<T> meas_sensor_val){
	// Do nothing
}

template <typename T>
void Ekf21Dof<T>::ComputeMeasNoiseJacobian(MatrixInv<T> meas_sensor_val){
	// Do Nothing
}

template <typename T>
void Ekf21Dof<T>::ComputeMeasFromState(MatrixInv<T> time_propagated_state){
